...
$$p_{M}(m)= \left(\left(1-p_1\right) \cdot \left(1-p_2\right)\right)^{m-1}  \cdot \left(1-\left(1-p_1\right) \cdot \left(1-p_2\right) \right)  \text{for mk=1,\cdots$$ 
...
![Automaton for HTT string](HTT.png)
...
![Automaton for HTH string](HTH.png)
...
**First conclussion**: Analyzing these automatons, it is possible to conclude that the average number of tosses for string $\text{[HTH]}$ is larger than for string $\text{[HTT]}$ because when both fail in state $q_2$, $\text{[HTH]}$ returns to the initial state $q_0$ and $\text{[HTT]}$ only goes back to $q_1$.
...
![Automaton for HTH string](trans_hth.PNG)

![Automaton for HTH string](trans_htt.PNG)
