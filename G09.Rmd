...
1. **src:** Various independent files which helped to do the simulation and create the images used in this .Rmd.
1. **G09.Rmd:** This file
1. **you.tsv:** The tab separated values used in part II.

## Repository
To fork repository or download a pdf version of this .Rmd go to [stat4DS-hw1](https://github.com/belgrades/stat4DS-hw1) 
...
|$a$|-1|0|1|2|
|:---:|:---:|:---:|:---:|:---:|
|$p\left( a \right)$|$\frac{1}{4}$| $\frac{1}{8}$  |$\frac{1}{8}$   | $\frac{1}{2}$  | 

and $p\left( a\right)=0$ for all other $a$. 

a. Let the random variable $Y$ be deﬁned by $Y = X^2$, i.e., if $X =2$,then $Y = 4$. Calculate the probability mass function of $Y$.
b. Calculate the value of the distribution functions of $X$ and $Y$ in $a=1$, $a=\frac{3}{4}$ and $a=\pi-1$.

#### Solution

a. As we define $Y = X^2$, we evaluate every element of the support of $X$ to obtain the image in $Y$.
    * if $X = -1$ then $Y = 1$.
    * if $X = 0$ then $Y = 0$.
    * if $X = 1$ then $Y = 1$
    * if $X = 2$ then $Y = 4$ 
    ...
    * $p_Y(4) = p(2) = \frac{1}{2}$
    ...
    |$a$|0|1|4|
    |:---:|:---:|:---:|:---:|
    |$p_Y\left( a \right)$|$\frac{1}{8}$| $\frac{3}{8}$ | $\frac{1}{2}$  | 
    
b. First, we calculate de distribution function of $X$ and $Y$. We named each as $F_X(a)$ and $F_Y(a)$ for $X$ and $Y$ respectively:

    |$a$|-1|0|1|2|
    |:---:|:---:|:---:|:---:|:---:|
    |$F_X(a)=\mathbb{P}\left(X \leq a\right)$|$\frac{1}{4}$| $\frac{3}{8}$  |$\frac{1}{2}$   | $1$  | 
    
    |$a$|0|1|4|
    |:---:|:---:|:---:|:---:|
    |$F_Y(a)=\mathbb{P}\left(Y \leq a\right)$|$\frac{1}{8}$| $\frac{1}{2}$ | $1$  | 
    
    * $F_{X}\left ( X\leq 1 \right ) = \frac{1}{2}$
    * $F_{Y}\left ( Y\leq 1 \right ) = \frac{1}{2}$
    * $F_{X}\left ( X\leq \frac{3}{4} \right ) = \frac{3}{8}$
    * $F_{Y}\left ( Y\leq \frac{3}{4} \right ) = \frac{1}{8}$
    * $F_{X}\left ( X\leq \pi - 3 \right ) = F_{X}\left ( X\leq 0.1415 \right ) =  \frac{3}{8}$
    * $F_{Y}\left ( Y\leq \pi - 3 \right ) = F_{X}\left ( Y\leq 0.1415 \right ) =  \frac{1}{8}$
...
We have defined $M$ as the number of times you participate in these lotteries until winning at least one prize.
We assume the probability of winning one contest is independent to the probability of winning the other and the probability of winning in any month is independent of the previous ones. 
...
Lets define events. $W_1: ``\text{ Win at least 1.000.000 euros in lottery 1}"$ and $W_2: ``\text{ Win at least 1.000.000 euros in lottery 2}"$ and assign probabilities $p_1$ and $p_2$ such that: $\mathbb{P}\left(W_1\right) = p_1$ and $\mathbb{P}\left(W_2\right) = p_2$.

The probability of failure is $\mathbb{P}\left(W_1^{c} \cap W_2^{c}\right) = \mathbb{P}\left(W_1^{c}\right) \cdot \mathbb{P}\left(W_2^{c}\right) = \left(1-p_1\right) \cdot \left(1-p_2\right)$

The parameter (probability of success) $p$ can be expressed as:

1. $p = 1 -\mathbb{P}\left( \text{failure}\right) = 1-\left(1-p_1\right) \cdot \left(1-p_2\right)$
1. $p = \mathbb{P}\left( W_1 \cup W_2 \right) = \mathbb{P}\left(W_1\right) + \mathbb{P}\left(W_2\right) - \mathbb{P}\left(W_1 \cap W_2 \right) = p_1+p_2 - p_1 \cdot p_2$
1. $p = \mathbb{P}\left(W_1 \cap W_2\right) + \mathbb{P}\left(W_1 \cap W_2^{c}\right) + \mathbb{P}\left(W_1^{c} \cap W_2\right) = p_{1} \cdot p_{2} + p_{1} (1 - p_{2})+ p_{2} (1 - p_{1})$

Finally, as an example the probability mass distribution associated to $M$ ~Geom($1-\left(1-p_1\right) \cdot \left(1-p_2\right)$) using the first parameter is:

$$p_{M}(m)= \left(\left(1-p_1\right) \cdot \left(1-p_2\right)\right)^{m-1}  \cdot \left(1-\left(1-p_1\right) \cdot \left(1-p_2\right) \right)  \text{for } m=1,2, \cdots$$ 
...
a. We can get $\mathbb{P}(X=2)$ only if we get one head in the first toss and another head in the second toss. Since the probability of getting a head is p, then: $\mathbb{P}(X=2) = p*p = p^2$
In addition, to have 2 heads in 3 tosses, it is necessary to get a tail before the second head. We have two possibilities $\left \{ HTH,THH \right\}$. So, $\mathbb{P}(X=3) = p(1-p)p + (1-p)p^2 = 2(1-p)p^2$. Finally, to get 2 heads in 4 tosses, we need to have 2 tails before the second head:  $\left \{HTTH,TTHH,THTH \right\}$. Which gives us:$\mathbb{P}(X=4) = p(1-p)(1-p)p + (1-p)(1-p)p^2 + (1-p)p(1-p)p = 3(1-p)^2p^2$

b. 
We know that we throw the coin until the second head appears. Given $n$ the number of throws, we will always have $2$ heads and $n-2$ tails. The probability of each event that has 2 heads and $n-2$ tails is given by $p^2 \cdot (1-p)^{n-2}$ assuming independence in each throw. Lastly, we need to count how many of these events exist. To do this, is important to notice that we end our experiment when we get our second head, as a result, the last throw will **always** be a head. Therefore, we need to count the ways 1 head and $n-2$ tails can be obtained in the first $n-1$ throws. This problem is equivalent to assign a position to the head given the $n-1$ spots available. This said, we need the number of subsets of size 1 from a set of size $n-1$ which is ${n-1 \choose 1}=n-1$. Finally, we have that $P(X=n)=(n-1) p^2 (1-p)^{n-2}$ with $n\geq2$.
...
Now imagine tossing a coin successively, and waiting till the first time a particular pattern appears, say
$\text{[HTT]}$. For example, if the sequence of tosses was:

\[\text{HHTHHTH[HTT]HHTTTHTH}\]

The pattern $\text{[HTT]}$ would first appear after the 10th toss. 

Now, consider these two different patterns $\text{[HTH]}$ and $\text{[HTT]}$.

In your opinion which of the following three options is true:

1. The average number of tosses until $\text{[HTH]}$ is **larger** than that until $\text{[HTT]}$.
2. The average number of tosses until $\text{[HTH]}$ is **the same** as that until $\text{[HTT]}$.
3. The average number of tosses until $\text{[HTH]}$ is **smaller** than that until $\text{[HTT]}$.

### Solution

At first sight, it seems that the answer should be **the same** because, assuming independence and a balanced coin, the probability of both patterns is the same: $\frac{1}{8}$. However, we'll show in 3  different ways that the average number of tosses until $\text{[HTH]}$ is actually **larger**  than that until $\text{[HTT]}$.

#### A computational approach

We can view the problem of finding the patterns as the [Substring Search](http://algs4.cs.princeton.edu/lectures/53SubstringSearch.pdf) problem. There exist several implementations to solve these problem. One of them, consists in obtaining a Deterministic finite state automaton (DFA) which accepts the string if the sequence of transitions reading every character leads to halt state (or final state). We now show both automaton in our context: 

![Automaton for HTT string](images/HTT.png)

This is the automaton for string $\text{[HTT]}$. Assuming the alphabet $\left \{\text{H,T} \right \}$. Every state transitions to other state if we read $\text{H}$ or $\text{T}$. 

1. In the initial state, $q_0$ we assume the empty string. 
1. In $q_1$ we assume that the last character we have read is $\text{H}$
1. In $q_2$ we assume that the last two characters we have read are $\text{HT}$
1. Finally, in $q_f$ we assume that we have read our 3 consecutive characters $\text{HTT}$ which means we arrived to our halt state.

![Automaton for HTH string](images/HTH.png)

This is the automaton for string $\text{[HTH]}$. Assuming the alphabet $\left \{\text{H,T} \right \}$. Every state transitions to other state if we read $\text{H}$ or $\text{T}$. 
...
It is important to notice that the difference between both automatons is on state $q_2$. In the case of $\text{[HTT]}$, if we failed to get the second consecutive $\text{T}$ we return to state $q_1$. Conversely, if we failed to obtain the $\text{H}$ we return to state $q_0$.

**First conclussion**: 

Analyzing these automatons, it is possible to conclude that the average number of tosses for string $\text{[HTH]}$ is larger than for string $\text{[HTT]}$ because when both fail in state $q_2$, $\text{[HTH]}$ returns to the initial state $q_0$ and $\text{[HTT]}$ only goes back to $q_1$.

#### "Simulating" our automatons:

Our first conclussion is only an naive guess. To support it with empirical evidence, we decided to program both automatons and analyze the average number of tosses until we get the desired string. They're programmed in Python and should run in Rmarkdown but we've decided to put the parameter ```eval=FALSE```.
...
1. ('HTH: ', 10.01468, 58.009584497599995)

**Second conclussion**: 

If we run our codes multiple times we concluded that the average number of tosses until we get the string $\text{[HTT]}$ is $\approx 8$ and the variance is $\approx 24$. However, for string $\text{[HTH]}$ the average number of tosses is $\approx 10$ and the variance is $\approx 58$. We then have a strong empirical evidence that the average number of tosses until string $\text{[HTH]}$ is larger than until $\text{[HTT]}$. 

#### A markov chain approach.
We may analyze our automaton as a Markov process in which the probability of transition between every state is the probability of obtaining characters $\text{H}$ or $\text{T}$. The transition matrices for both automatons are:

![Automaton for HTH string](images/trans_hth.PNG)

![Automaton for HTH string](images/trans_htt.PNG)

In both cases, we have an absorbent state $q_f$ in which the probability of staying in the state is 1 when we arrive to this state and transient states $\left \{q_0, q_1, q_2 \right \}$. Read [Classification of states](https://www.math.ucdavis.edu/~gravner/MAT135B/materials/ch13.pdf) for further information about classification of states.

This said, we investigated and this type of matrices are called Absorbent Markov Chains. In the chapter III of [Finite Markov Chains](http://www.springer.com/us/book/9780387901923),(Kennedy and Snell, 1976, Springer-Velag). We found several theorems which lead to 3 important calculations:

1. The fundamental matrix $N$ is obtained using $N= \left(I_t -Q \right)^-1$. Where $Q$ is the transition matrix of the transient states $\left \{q_0, q_1, q_2 \right \}$.
1. The expected number of steps before being absorbed when starting in transient state $i$ is the ith entry of the vector: $t = N \cdot 1$ with $1$ a 1 column vector.
1. The variance on the number of steps before being absorbed when starting in transient state $i$ is the ith entry of the vector: $2\left( 2N -I_t\right)t-t_{sq}$ where $t_{sq}$ is the [Hadamard Product](http://buzzard.ups.edu/courses/2007spring/projects/million-paper.pdf) of $t$ with itself.

Now, lets see the calculations for the string  $\text{[HTT]}$:

![](images/complete_htt.PNG)

As we can see, the average number of tosses for string $\text{[HTT]}$ when we start in state $q_0$ is 8. Also, the variance is 24. If we do similar calculations for the markov chain related to string $\text{[HTH]}$, we get that the average number of tosses is 10 and the variance is 58.

**Third and final conclussion**:

We got similar numbers between our simulation and the actual theoretical number using Absorbent Markov Chains. This said, we arrived to the same conclussion in 3 different ways which is, surprisingly, that **the average number of tosses until $\text{[HTH]}$ is larger than until $\text{[HTT]}$ (It's actually 10 vs 8).
